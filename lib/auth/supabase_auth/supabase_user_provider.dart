...
class DairyManagementSystemSupabaseUser extends BaseAuthUser {
  DairyManagementSystemSupabaseUser(this.user);
  User? user;
  @override
...
Stream<BaseAuthUser> dairyManagementSystemSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = DairyManagementSystemSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
